...
def get_parametric_default_configs():
  ...
  config.train_bs_x = 50
  config.train_bs_t = 50
  config.num_stage = 10
  config.num_epoch = 6
  config.num_itr = 120
  config.T = 1.0
  ...
  config.problem_name = 'parametric'
 ...
 # cond dist
  ...
  config.output_size = (1,2)
  config.permute_batch = True
  config.imputation_eval = True
#   config.output_layer = 'conv1d'  # 'conv1d_silu'

  model_configs={
      'mlp':get_mlp_config(config.input_size, config.output_size),
      'Unetv2':get_Unetv2_config(config.input_size),
      'Transformerv2':get_Transformerv2_config(config.input_size)
